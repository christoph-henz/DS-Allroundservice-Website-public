...
class AdminAPI extends Page {
    ...
    private function getStats() {
        try {
            ...
            $monthlySubmissions = $this->queryAll(
                "SELECT COUNT(*) as count FROM questionnaire_submissions 
                 WHERE DATE_FORMAT(submitted_at, '%Y-%m') = DATE_FORMAT(NOW(), '%Y-%m')"
            )[0]['count'] ?? 0;
                ...
                if ($pdfResult['success']) {
                    ...
                    $stmt = $this->db->prepare(
                        "UPDATE questionnaire_submissions 
                         SET pdf_generated = 1, pdf_path = ?, processed_at = NOW()
                         WHERE id = ?"
                    );
    ...
    /**
     * Normalize and format German phone numbers
     */
    private function normalizePhoneNumber($phoneNumber) {
        ...
        $cleanPhone = preg_replace('/[^\d\+\s\-\(\)]/', '', trim($phoneNumber));
        
        // Remove common formatting characters
        $cleanPhone = str_replace([' ', '-', '(', ')'], '', $cleanPhone);
        
        // Handle German phone number patterns
        if (preg_match('/^(\+49|0049|0)(.+)$/', $cleanPhone, $matches)) {
            $number = $matches[2];
            
            // Format as +49 xxx xxxxxxx
            if (strlen($number) >= 10) {
                // Mobile numbers (015x, 016x, 017x, etc.)
                if (preg_match('/^(15|16|17)(\d{8,9})$/', $number, $mobileMatches)) {
                    return '+49 ' . $mobileMatches[1] . ' ' . chunk_split($mobileMatches[2], 4, ' ');
                }
                // Landline numbers
                else {
                    return '+49 ' . $number;
        ...
        // Return cleaned number if no specific pattern matched
        return $cleanPhone;
    }
    
    protected function extractContactDataFromAnswers($answers) {
        $contactData = [
            'first_name' => null,
            'last_name' => null,
            'name' => null,
        ...
        if (is_array($answers) && isset($answers[0]) && is_array($answers[0])) {
            ...
            foreach ($answers as $answer) {
                ...
                // Extract first name
                if (strpos($questionText, 'vorname') !== false && strpos($questionText, 'nachname') === false) {
                    $contactData['first_name'] = $answerText;
                }
                
                // Extract last name
                if (strpos($questionText, 'nachname') !== false && strpos($questionText, 'vorname') === false) {
                    $contactData['last_name'] = $answerText;
                }
                
                // Extract combined name (vor- und nachname)
                if (strpos($questionText, 'vor-') !== false && strpos($questionText, 'nachname') !== false) {
                    $contactData['name'] = $answerText;
                    // Try to split into first and last name if not already set
                    if (!$contactData['first_name'] && !$contactData['last_name']) {
                        $nameParts = explode(' ', $answerText, 2);
                        if (count($nameParts) >= 2) {
                            $contactData['first_name'] = trim($nameParts[0]);
                            $contactData['last_name'] = trim($nameParts[1]);
                        }
                    }
                ...
                if (strpos($questionText, 'telefon') !== false || 
                    strpos($questionText, 'phone') !== false ||
                    strpos($questionText, 'handy') !== false ||
                    strpos($questionText, 'mobil') !== false ||
                    strpos($questionText, 'mobile') !== false ||
                    strpos($questionText, 'tel.') !== false ||
                    strpos($questionText, 'tel:') !== false ||
                    strpos($questionText, 'rufnummer') !== false ||
                    strpos($questionText, 'kontakt') !== false && strpos($questionText, 'nummer') !== false) {
                    $contactData['phone'] = $this->normalizePhoneNumber($answerText);
                }
            }
        } else {
            // Old format: associative array with field keys
            $firstNameFields = [
                'vorname', 'first_name', 'firstname', 'kunde_vorname', 'vname'
            ];
            
            $lastNameFields = [
                'nachname', 'last_name', 'lastname', 'kunde_nachname', 'nname', 'familienname'
            ];
            ...
            $phoneFields = [
                ...
                'kontakt_telefon', 'kunde_telefon', 'telefon_nummer', 'mobilnummer',
                'mobil', 'handynummer', 'rufnummer', 'kontaktnummer', 'phone_number',
                'mobile_number', 'cell_phone', 'cellphone', 'tel_nummer', 'tel_nr',
                'festnetz', 'festnetznummer', 'privatnummer', 'geschaeftsnummer'
            ];
            
            // Extract first name
            foreach ($firstNameFields as $field) {
                if (isset($answers[$field]) && is_string($answers[$field]) && !empty(trim($answers[$field]))) {
                    $contactData['first_name'] = trim($answers[$field]);
                    break;
                }
            }
            ...
            foreach ($lastNameFields as $field) {
                if (isset($answers[$field]) && is_string($answers[$field]) && !empty(trim($answers[$field]))) {
                    $contactData['last_name'] = trim($answers[$field]);
                    break;
                }
            }
            
            // Extract combined name
            ...
            foreach ($phoneFields as $field) {
                if (isset($answers[$field]) && is_string($answers[$field]) && !empty(trim($answers[$field]))) {
                    $contactData['phone'] = $this->normalizePhoneNumber(trim($answers[$field]));
                    break;
                }
            }
        }
    ...
    private function updateSubmissionStatus() {
        ...
        try {
            $stmt = $this->db->prepare("UPDATE questionnaire_submissions SET status = ?, internal_notes = ?, processed_at = NOW() WHERE id = ?");
    ...
    private function generateOfferPDF() {
        ...
        try {
            ...
            $pdfGenerator = new OfferPDFGenerator($this->db);
            ...
            if (in_array($submission['status'], ['neu', 'new', 'in_bearbeitung'])) {
                $this->db->prepare("UPDATE questionnaire_submissions SET status = 'angebot_erstellt', processed_at = NOW() WHERE id = ?")
                    ->execute([$id]);
            }
        ...
        try {
            $this->execute(
                "UPDATE questionnaires 
                 SET title = ?, service_id = ?, description = ?, status = ?, updated_at = NOW()
                 WHERE id = ?",
        ...
        try {
            $this->execute(
                "UPDATE questions_simple 
                 SET question_text = ?, question_type = ?, is_required = ?, placeholder_text = ?, help_text = ?, options = ?, validation_rules = ?, updated_at = NOW()
                 WHERE id = ?",
    ...
    private function createGroupFromQuestions() {
        ...
        try {
            ...
            // First, get the next sort order
            $sortOrderStmt = $this->query(
                "SELECT COALESCE(MAX(sort_order), 0) + 1 as next_sort_order FROM question_groups WHERE questionnaire_id = ?",
                [$questionnaireId]
            );
            ...
            $groupStmt = $this->query(
                "INSERT INTO question_groups (questionnaire_id, name, description, sort_order, created_at) 
                 VALUES (?, ?, ?, ?, NOW())",
                [$questionnaireId, $name, $description, $nextSortOrder]
            );
    ...
    private function getArchivedSubmissions() {
        try {
            ...
            // Only count sent (1) or accepted (2) offers, with accepted prioritized
            $sql = "SELECT qs.*, s.name as service_name,
                           COUNT(DISTINCT best_offers.offer_id) as offers_count,
                           SUM(best_offers.total_gross) as total_offers_value
                    FROM questionnaire_submissions qs
                    LEFT JOIN services s ON qs.service_id = s.id
                    LEFT JOIN (
                        SELECT o.id as offer_id, o.submission_id, o.total_gross,
                               ROW_NUMBER() OVER (
                                   PARTITION BY o.submission_id 
                                   ORDER BY 
                                       CASE o.status 
                                           WHEN 2 THEN 1 
                                           WHEN 1 THEN 2 
                    ...
                    ) as best_offers ON qs.id = best_offers.submission_id AND best_offers.rn = 1
                    WHERE {$dateCondition}";
    ...
    private function getArchiveStats() {
        try {
            ...
            // Get archived offers stats (only sent=1 or accepted=2 offers)
            // Note: If multiple offers exist for same submission, 'accepted' (2) is prioritized over 'sent' (1)
            $stmt = $this->db->prepare(
                "SELECT COUNT(DISTINCT best_offers.submission_id) as archived_offers,
                        SUM(best_offers.total_gross) as total_archived_value,
                        COUNT(DISTINCT best_offers.submission_id) as submissions_with_offers
                 FROM (
                     SELECT o.submission_id, o.total_gross,
                            CASE o.status 
                                WHEN 2 THEN 1 
                                WHEN 1 THEN 2 
            ...
            // Get stats by service (only sent=1 or accepted=2 offers, accepted prioritized)
            $stmt = $this->db->prepare(
                "SELECT s.name as service_name,
                        COUNT(DISTINCT qs.id) as submissions_count,
                        COUNT(DISTINCT best_offers.offer_id) as offers_count,
                        SUM(best_offers.total_gross) as service_value
                 FROM questionnaire_submissions qs
                 LEFT JOIN services s ON qs.service_id = s.id
                 LEFT JOIN (
                     SELECT o.id as offer_id, o.submission_id, o.total_gross,
    ...
    private function saveSettings() {
        ...
        try {
            ...
            foreach ($settings as $setting) {
                ...
                $result = $this->execute(
                    "REPLACE INTO settings (setting_key, setting_value, setting_type, description, is_public, updated_at) 
        ...
        try {
            ...
            foreach ($requiredSettings as $key => $defaultValue) {
                ...
                if (!$existing) {
                    ...
                    $this->query(
                        "INSERT IGNORE INTO settings (setting_key, setting_value, setting_type, description, created_at, updated_at) VALUES (?, ?, 'string', ?, NOW(), NOW())",
    ...
    private function logEmailSent($templateKey, $toEmail, $subject, $variables) {
        try {
            $this->query(
                "INSERT INTO email_logs (template_key, recipient_email, subject, variables_used, sent_at, created_at) VALUES (?, ?, ?, ?, NOW(), NOW())",
        ...
        try {
            $sql = "SELECT us.created_at, u.username, us.ip_address, us.expires_at, 
                    CASE 
                        ...
                        WHEN us.expires_at <= NOW() THEN 'Abgelaufen'
                        ELSE 'Aktiv'
                    END as status
                    FROM user_sessions us 
                    LEFT JOIN users u ON us.user_id = u.id 
    ...
    private function terminateSession() {
        error_log("🔍 terminateSession() called");
        
        // Check if user is Admin (not just logs_view permission)
        $currentRole = $_SESSION['role'] ?? '';
        error_log("🔍 Current role: {$currentRole}");
        
        if ($currentRole !== 'Admin') {
            error_log("❌ Access denied - Role: {$currentRole}");
            return $this->error('Zugriff verweigert. Nur Administratoren können Sessions beenden.');
        }
        
        $data = json_decode(file_get_contents('php://input'), true);
        $sessionId = $data['session_id'] ?? '';
        
        error_log("🔍 Session ID to terminate: {$sessionId}");
        ...
        try {
            ...
            $stmt = $this->db->prepare("
                SELECT us.user_id, u.username, us.is_active, us.expires_at
                FROM user_sessions us
                LEFT JOIN users u ON us.user_id = u.id
                WHERE us.session_token = :session_id
            ");
            $stmt->execute([':session_id' => $sessionId]);
            $sessionInfo = $stmt->fetch();
            
            error_log("🔍 Session info: " . print_r($sessionInfo, true));
            
            if (!$sessionInfo) {
                error_log("❌ Session not found in database");
                return $this->error('Sitzung nicht gefunden (Session-ID: ' . substr($sessionId, 0, 10) . '...)');
            }
            
            // Check if session is already inactive
            if ($sessionInfo['is_active'] == 0) {
                error_log("⚠️ Session is already inactive");
                return $this->error('Sitzung ist bereits inaktiv');
            }
            
            // Mark session as expired immediately and set is_active to 0
            $stmt = $this->db->prepare("
            ...
            if ($rowCount > 0) {
                ...
                $targetUsername = $sessionInfo['username'] ?? 'Unknown User';
                error_log("✅ Admin '{$adminUsername}' terminated session for user '{$targetUsername}' (Session: {$sessionId})");
                
                return $this->success([
                    'message' => 'Sitzung erfolgreich beendet',
                    'terminated_user' => $targetUsername
                ]);
            } else {
                error_log("❌ No rows were updated - Session might not exist");
                return $this->error('Fehler beim Beenden der Sitzung - Keine Zeilen betroffen');
            }
        } catch (PDOException $e) {
            error_log("❌ PDOException in terminateSession: " . $e->getMessage());
            error_log("Stack trace: " . $e->getTraceAsString());
            return $this->error('Datenbankfehler: ' . $e->getMessage());
        } catch (Exception $e) {
            error_log("❌ Exception in terminateSession: " . $e->getMessage());
            return $this->error('Serverfehler: ' . $e->getMessage());
        }
    }
